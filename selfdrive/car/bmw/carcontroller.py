  ...
  def update(self, CC, CS, now_nanos):
    ...
    if not cruise_stalk_human_pressing:
      if self.cruise_cancel and CS.out.cruiseState.enabled:
        ...
        if actuators.accel < -0.7 and self.calc_desired_speed <= self.min_cruise_speed: # hack: cruise speed target cannot go below minCruiseSpeed, so it's safe to hold minus 5
