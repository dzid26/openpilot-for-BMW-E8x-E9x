...
# human intervention detection with stepper motors actuators with variable holding torque
# carcontroller requests already commands centering torque that holds calibrated centering torque + TqOffset margin (related to constant steering friction)
# stepper can lose steps if its current is saturated or a road disturbance or driver intervention is present
...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), vin="", has_relay=False):
    ...
    if candidate in [CAR.E82_DCC, CAR.E90_DCC]:  # DCC has higher threshold
      ret.minEnableSpeed = 28. * CV.KPH_TO_MS if is_metric else 20. * CV.MPH_TO_MS #todo merge with carstate is_metric
  ...
  def update(self, c, can_strings):
    ...
    ret = self.CS.update(self.cp, self.cp_cam, self.cp_aux)

    ret.canValid = self.cp.can_valid and self.cp_cam.can_valid and self.cp_aux.can_valid
    ...
    if ret.vEgo < self.CP.minEnableSpeed-1 and self.CP.openpilotLongitudinalControl:
